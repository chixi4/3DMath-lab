...
RAW_DIR="$REPO_ROOT/output/previews-raw"
PREVIEW_DIR="$REPO_ROOT/output/previews-16x9"
CHECK_DIR="$REPO_ROOT/output/preview-checks"
VIEW_MODE="${MANIM_VIEW:-fixed}"
...
for i in {1..6}; do
  ...
  MANIM_VIEW="$VIEW_MODE" MANIM_THEME="$THEME" manim -qk -s -r 1280,720 --disable_caching src/prototype-manimce/revolve-slice-prototype.py "$scene_name"
...
ffmpeg -y \
  ...
  -frames:v 1 \
  -update 1 \
  "$CHECK_DIR/contact_6up_16x9.png" \
  >/dev/null 2>&1
